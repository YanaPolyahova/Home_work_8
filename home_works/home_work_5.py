...
# Перечень документов, которые обнаруживаются, обнаруживаются в следующем виде:
...
def add_documents(): #команда a
    ...
    if new_doc not in documents:
        ...
        if choosen_shelf in directories:
            ...
            return 'Введите корректный номер полки'
    else:
        return 'Данный документ уже добавлен'

def delete_doc(): #команда d
    number_doc = input('Введите номер документа: ')
    d = 0
    for document in documents:
        if number_doc in document.values():
